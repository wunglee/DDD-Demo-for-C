...
class 拦截器管理器 {
private:
    std::list<请求拦截器*> 拦截器链表;
    void 前置拦截(std::string 方法名,const boost::any &参数,const std::list<std::string> &拦截器配置, std::stack<请求拦截器*> &调用栈) {
        for(请求拦截器* 请求拦截器_:拦截器链表){
    ...
    处理结果 &后置拦截(std::string 方法名, const boost::any &参数,处理结果 &处理结果_, std::stack<请求拦截器*> &调用栈) const {
        while(!调用栈.empty()){
...
public:
    拦截器管理器(const std::list <请求拦截器*> &拦截器链表) : 拦截器链表(拦截器链表) {}
    处理结果 拦截处理(请求处理器 &请求处理器_,std::string 方法名,const boost::any &参数,const std::list <std::string> &拦截器配置){
        try{
            处理结果 处理结果_;
            std::stack<请求拦截器*> 调用栈;
