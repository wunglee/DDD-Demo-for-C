...
private:
    ...
    转账仓储接口* 转账仓储接口_;
    转账查询接口* 转账查询服务接口_;
    转账服务(用户服务接口 *用户服务,转账仓储接口 *转账仓储接口, 转账查询接口 * 转账查询服务接口):
    用户服务_(用户服务),转账仓储接口_(转账仓储接口),转账查询服务接口_(转账查询服务接口){
    }
    转账服务(转账服务& 转账服务){
        this->转账查询服务接口_=转账服务.转账查询服务接口_;
        this->转账仓储接口_=转账服务.转账仓储接口_;
    }
...
public:
    ...
    static 转账服务* 构建单例(
        ...
        if (转账服务::转账服务_ == nullptr) {
            转账服务::转账服务_ = new 转账服务(用户服务_, 转账仓储接口_, 转账查询接口_);
        }
        return 转账服务::转账服务_;
    }
    static 转账服务* 获取单例(){
    ...
    virtual int 申请转账(std::string 付款账号, std::string 收款账号, float 金额) override{
        转账用户 付款账号_=转账用户(付款账号, 用户服务_->是否已冻结(付款账号));
        转账用户 收款账号_=转账用户(收款账号, 用户服务_->是否已冻结(收款账号));
