...
private:
    转账服务接口 * 转账服务接口_;
    static 转账命令处理器* 转账命令处理器_;
    转账命令处理器(转账服务接口 *转账服务接口_) : 转账服务接口_(转账服务接口_) {}
public:
    virtual ~转账命令处理器(){
    }
    static 转账命令处理器* 构建单例(转账服务接口 * 转账服务接口){
        if (转账命令处理器::转账命令处理器_ == nullptr) {
            转账命令处理器::转账命令处理器_ = new 转账命令处理器(转账服务接口);
        }
        return 转账命令处理器::转账命令处理器_;
    }
    static 转账命令处理器* 获取单例(){
        if (转账命令处理器::转账命令处理器_ == nullptr) {
            throw "单例未构建";
