...
private:
    ...
    资金帐户服务接口 * 资金帐户命令服务接口_= nullptr;
    资金事件处理器(资金帐户服务接口 * 资金帐户命令服务接口): 资金帐户命令服务接口_(资金帐户命令服务接口){}
public:
    ~资金事件处理器() {
        资金事件处理器::资金事件处理器_= nullptr;
    }
    static 资金事件处理器* 构建单例(资金帐户服务接口 * 资金帐户命令服务接口){
        if (资金事件处理器::资金事件处理器_ == nullptr) {
            资金事件处理器::资金事件处理器_ = new 资金事件处理器(资金帐户命令服务接口);
        }
    ...
    void 处理(const 领域事件 领域事件_){
        ...
        if(事件名=="转账已创建"){
                ...
                资金帐户命令服务接口_->扣减资金(转账已创建事件内容_.转账单号,转账已创建事件内容_.付款方账号,转账已创建事件内容_.付款金额);
        }
        if(事件名=="用户已创建") {
            事件内容 = 领域事件_.获取内容();
            用户已创建DTO 用户已创建事件内容_ = boost::any_cast<用户已创建DTO>(事件内容);
            资金帐户命令服务接口_->添加资金帐户(用户已创建事件内容_.用户名);
        }
        if(事件名=="转账已出账") {
            事件内容 = 领域事件_.获取内容();
            转账已出账DTO 转账已出账事件内容_ = boost::any_cast<转账已出账DTO>(事件内容);
            资金帐户命令服务接口_->增加资金(转账已出账事件内容_.转账单号, 转账已出账事件内容_.收款方账号, 转账已出账事件内容_.金额);
        }
        if(事件名=="转账退款中") {
            事件内容 = 领域事件_.获取内容();
            转账退款事件 转账退款事件内容_ = boost::any_cast<转账退款事件>(事件内容);
            资金帐户命令服务接口_->增加资金(转账退款事件内容_.单号, 转账退款事件内容_.退回账号, 转账退款事件内容_.金额);
        }
    };
};
