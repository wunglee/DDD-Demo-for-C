...
void 创建用户(用户服务接口 * 用户服务接口_,std::string 付款方,bool 冻结付款方,std::string 收款方,bool 冻结收款方){
    用户服务接口_->添加用户(付款方);
    用户服务接口_->添加用户(收款方);
    if(冻结付款方)
        用户服务接口_->冻结用户(付款方);
...
void 创建用户(用户服务接口 * 用户服务接口_,std::string 付款方,std::string 收款方){
    创建用户(用户服务接口_,付款方,false,收款方,false);
}
void 检查转账状态(int 单号, 转账::状态 状态_){
    boost::optional<转账> 转账_ = 转账服务::获取单例()->获取转账(单号);
    ASSERT_EQ(转账_.has_value(),true);
    ASSERT_EQ(转账_.value().获取状态(),状态_);
}
int 申请转账(用户服务接口 * 用户服务接口_,std::string 付款方,std::string 收款方,float 金额){
