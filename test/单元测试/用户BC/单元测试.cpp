...
TEST_F(主适配器请求测试环境, 通过主适配器添加用户){
    用户HTTP服务端::获取单例()->添加用户("XX");
    ASSERT_EQ(用户服务模拟::获取单例()->获取用户("XX").has_value(), true);
}
TEST_F(主适配器请求测试环境, 通过主适配器获取用户){
    用户HTTP服务端::获取单例()->添加用户("XX");
    ASSERT_EQ(用户HTTP服务端::获取单例()->获取用户("XX").has_value(), true);
}
TEST_F(主适配器请求测试环境, 通过主适配器冻结用户){
    用户HTTP服务端::获取单例()->添加用户("XX");
    用户HTTP服务端::获取单例()->冻结用户("XX");
    ASSERT_EQ(用户服务模拟::获取单例()->获取用户("XX")->是否冻结(), true);
}
TEST_F(主适配器请求测试环境, 通过主适配器解冻用户){
    用户HTTP服务端::获取单例()->添加用户("XX");
    用户HTTP服务端::获取单例()->冻结用户("XX");
    用户HTTP服务端::获取单例()->解冻用户("XX");
    ASSERT_EQ(用户服务模拟::获取单例()->获取用户("XX")->是否冻结(), false);
}
TEST_F(主适配器请求测试环境, 通过主适配器获取冻结状态){
    用户HTTP服务端::获取单例()->添加用户("XX");
    用户HTTP服务端::获取单例()->冻结用户("XX");
    ASSERT_EQ(用户HTTP服务端::获取单例()->是否已冻结("XX"), true);
}