...
TEST_F(命令总线测试环境1, 提交命令){
    测试提交命令();
}
TEST_F(命令总线测试环境2, 拦截命令){
    测试提交命令();
    ASSERT_EQ(命令拦截器模拟1_->执行前调用, true);
    ASSERT_EQ(命令拦截器模拟1_->执行后调用, true);
    ASSERT_EQ(命令拦截器模拟2_->执行前调用, true);
    ASSERT_EQ(命令拦截器模拟2_->执行后调用, true);
}
